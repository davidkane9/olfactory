...
```{r}
ggplot(targets, aes(x = t_ideo_scale)) + 
  geom_bar() +
...
```{r}
ggplot(evaluators, aes(x = e_ideo_scale)) + 
  geom_bar() +
...
Ought to use `stargazer` or `kableExtra` to format these regressions results nicely. For now, I just show the raw numbers. For now, I don't bother with clustered standard errors.


## Model 1

```{r}
summary(lm(data = x, attractive ~ same_ideology + same_sex + 
             e_ideology + t_ideology +
             e_gender   + t_gender +
...
```{r echo = TRUE}
summary(lm(data = x, attractive ~ neg_abs_ideo_diff + same_sex + e_ideology + t_ideology + 
             e_gender + t_gender + e_attractiveness + t_attractiveness))
```


## Model 3

I can replicate Model 3 very closely, although I am not currently using clustered standard errors.

```{r echo = TRUE}
summary(lm(data = x, attractive ~ neg_abs_ideo_diff + same_sex + t_id + e_id))
```

Note how closely the key result, the coefficient and standard error for `neg_abs_ideo_diff` matches the published version.
